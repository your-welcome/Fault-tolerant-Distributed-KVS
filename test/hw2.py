...
class TestHW2(unittest.TestCase):
    ...
    def spin_up_nodes(self):
        ...
        subprocess.call("docker build -t assignment2 ..", shell=True)
        exec_string_main = sudo + " docker run -p 8083:8080 --name aasim --net=mynet --ip=10.0.0.20 -d %s" % contname
        print(exec_string_main)
        self.__class__.node_ids.append(subprocess.check_output(exec_string_main, shell=True).strip('\n'))

        exec_string_forw1 = sudo + " docker run -p 8084:8080 --name aasim1 --net=mynet -e MAINIP=10.0.0.20:8080 -d %s" % contname
        print(exec_string_forw1)
        self.__class__.node_ids.append(subprocess.check_output(exec_string_forw1, shell=True).strip('\n'))

        exec_string_forw2 = sudo + " docker run -p 8085:8080 --name aasim2 --net=mynet -e MAINIP=10.0.0.20:8080 -d %s" % contname
